...
//     const gameLogic = GameLogic(player1, player2, board);
    
//     const getBoard = () => board.getBoard();
//     const getActivePlayer = () => gameLogic.getCurrentPlayer();
...
//                 board.mark(row, col, activePlayer.symbol);
...
//     return { getBoard, getActivePlayer, playRound };
// }
...
function GameController() {
    ...
    const playRound = (col, row) => {
        ...
        if (board.getBoard()[row][col] === 0) { // Ensure the cell is empty
            board.mark(row, col, activePlayer.symbol);
...
function ScreenController() {
    ...
    document.body.prepend(playerTurnDiv);
    ...
    boardDiv.addEventListener("click", (e) => {
        ...
        if (isNaN(row) || isNaN(col)) return; // Ensure valid selection
